...
\begin{figure}[hbt]
<<dependson='omni-data', echo=FALSE, fig.width=9, fig.height=8, out.width='\\columnwidth', cache=TRUE, results='asis'>>=
qplot(dmin/60+8, Freq, data=subset(domni, id=="others"), colour=I("grey60"), facets=day~location) +
  geom_point(aes(colour=id), subset(domni, id!="others")) + ggtitle("Messages sent from two special ids by minute") + 
xlab("Time of the day") + ylab("Number of total messages by minute") + theme_bw() +
  scale_colour_brewer(palette="Set2") + 
  theme(legend.position="bottom", plot.margin=unit(c(0,0,0,0), units="cm"))
@
\caption{\label{fig:overview} Two IDs are notable for their large volume of messages: 1278894 and 839736. These IDs are responsible for almost 80\% of the message volume. Both these ids are stationary, sending messages from the Entry Corridor only. 
}
\end{figure}
