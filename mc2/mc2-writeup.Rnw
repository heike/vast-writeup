...
\shortauthortitle{Hofmann \MakeLowercase{\textit{et al.}}: Visualizing Communication Patterns}
...
\keywords{Spatio-temporal visualizations; Barcode plot; }
...
\begin{figure}
\centering
<<kiddies-network, dependson='kiddies-data', echo=FALSE, fig.width=6, fig.height=6, out.width='0.65\\columnwidth', cache=TRUE>>=
set.seed(20150814)
...
qplot(x=dmin, xend=dmin+timespent/60^2, y=id, yend=id, data=subset(alldata, day=="Sun" & group %in% c(2566)), colour=Type,geom="segment", size=I(3)) +  theme_bw() + scale_colour_manual(values=cols[c(13, 3,4,11, 7,1,8)]) +
...
@
\subcaption{\label{fig:x38}  The `penguins' come into DinoFun World at around 9:30, but take their first ride after 12:30 pm - before that they spend a long time in or around Creighton Pavilion, however they never check into it!  }
\end{subfigure}
\begin{subfigure}{\columnwidth}
\centering
...
When investigating this group if 37 further we find some interesting things:  the sensors do not pick up on any movements for any one of the 37 members of the group between 9:45 until noon.
