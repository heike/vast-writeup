...
\subsection{A scene worthwhile texting about}
...
\begin{figure}
<<kiddies-map, dependson='kiddies-data', echo=FALSE, fig.width=6, fig.height=6.25, out.width='\\columnwidth', cache=TRUE>>=
ggimage(park, fullpage = FALSE) +
  ...
  scale_size("Number of messages sent") + 
  theme(legend.position="bottom") + ggtitle("News from the SabreTooth Theatre")
@
\caption{\label{fig:map}Messages sent during the spike originating in the Kiddie Land. Maps at 15 second intervals show that some news-worthy event happened in front of the SabreTooth Theatre (location 64), from there people spread to the North and the South and then slowly dissipate.}
\end{figure}

\begin{figure}
\centering
<<kiddies-network, dependson='kiddies-data', echo=FALSE, fig.width=6, fig.height=6, out.width='0.8\\columnwidth', cache=TRUE>>=
set.seed(20150814)
...
XXX We need a good name for the 37 guys from group 2566 - neither one is good XXX
...
\begin{figure}
<<messages, echo=FALSE, fig.width=7, fig.height=4, out.width='\\columnwidth'>>=
...
@
\caption{\label{fig:messages} Messages sent from the chaps of group 2566. Between 11:30 and 11:45 am there is a flurry of messages among the members of the group.  About half of them take part in the Cindysaurus Trivia game. }
\end{figure}
...
<<x38, echo=FALSE, fig.width=7, fig.height=4.25, out.width='\\columnwidth'>>=
...
qplot(x=dmin, xend=dmin+timespent/60^2, y=id, yend=id, data=subset(checkinsgr, day=="Sun" & group %in% c(2566)), colour=Type,geom="segment", size=I(3)) +  theme_bw() + scale_colour_manual(values=cols[c(3,4,11, 7,1,8)]) +
  ggtitle("Activities of the 37 guys from group 2566") + theme(legend.position="bottom") + ylab("") + guides(colour = guide_legend(nrow=2)) +
...
They are either standing perfectly still in the same 5m x 5m cell (tile 32, 33), or they found a hole in the park's sensor system and are on the loose somewhere around the Creighton Pavilion, into which they conveniently fail to check in (see figure~\ref{fig:x38}).
