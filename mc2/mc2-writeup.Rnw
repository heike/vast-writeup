...
They are either standing perfectly still in the same 5m x 5m cell (tile 32, 33), or they found a hole in the park's sensor system and are on the loose somewhere around the Creighton Pavilion (see figure~\ref{fig:map}).
...
%'   movfri <- sunmov[,c("Timestamp", "id", "type", "X", "Y")]
%'   comfri <- coaster[, c("Timestamp", "from", "to", "location")]
%'   movfri$to <- NA
%'   movfri$location <- NA
%'   
%'   names(comfri)[2] <- "id"
