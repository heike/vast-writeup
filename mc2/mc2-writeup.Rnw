...
eval=TRUE>>=
qplot(dmin/60+8, Freq, data=subset(domni, id=="others"), geom=c("point", "line"), colour=I("grey60")) +
  facet_grid(facets=day~location, scale="free_y", space="free_y") +
  geom_point(aes(colour=id), subset(domni, id!="others")) +
  geom_line(aes(colour=id, group=id), size=0.5, alpha=0.5, subset(domni, id!="others")) +
