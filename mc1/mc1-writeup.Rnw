...
xybymin <- move %>% filter(type=="check-in") %>% group_by(day, hour, min, X, Y) %>% summarize(n=n())
xybymin$X <- as.numeric(as.character(xybymin$X))
xybymin$Y <- as.numeric(as.character(xybymin$Y))
xybymin <- merge(xybymin, park, by=c("X", "Y"))
xybytype <- xybymin %>% group_by(day, hour, min, Type) %>% summarize(n=sum(n))

qplot(facets=day~., colour=Name, hour+min/60, n, data=subset(xybymin, Name %in% c("Creighton Pavilion", "concert space"))) + theme_bw() + 
  theme(legend.position="bottom") + xlab("Time of the day") + ylab("Number of check-ins in each of the areas by minute") + ggtitle("Check-ins to Scott Jones related entertainment") +
