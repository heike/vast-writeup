...
\maketitle
here we need to talk about a few things: park.csv
...
\begin{figure}
<<timeinthrill, dependson='checkins', echo=FALSE, fig.width=9, fig.height=8, out.width='\\columnwidth', cache=TRUE, results='asis'>>=

ggplot(data=subset(checkins, Type=="Thrill Rides")) +
...
@
\caption{\label{fig:timeinthrill} Time spent in each of the Thrill Rides.}
\end{figure}
...
We define a group as a set of park goers who do rides together.
...
<<vandals, dependson='vandalsdata', echo=FALSE, results='hide', fig.keep=TRUE, fig.width=10, fig.height=6>>=
ggplot(data=scott) +
  geom_rect(ymax=200, xmin=9.5, xmax=11.5, ymin=0, fill="grey90", alpha=0.5) +
  geom_rect(xmin=14.5, xmax=16.5, ymin=0, ymax=200, fill="grey90", alpha=0.5) +
  geom_errorbarh(aes(x=(hour*60+min)/60, xmin=(hour*60+min)/60,
                     xmax=(hour*60+min+timespent/60)/60, 
  ...
  geom_errorbarh(aes(x=(hour*60+min)/60, xmin=(hour*60+min)/60,
                 ...
                 data=vandals) + 
#  geom_line(aes(y=timeest), colour="red") + 
...
scale_x_continuous("Time of the day", breaks=8:23, labels=paste(c(8:12,1:11), c(rep("am", 4), rep("pm", 12)))) + 
  ...
  geom_text(aes(x=(hour*60+min)/60, y=timespent/60, 
                label=label), colour="orange", data=vandals2, hjust=-0.1, 
            vjust=1.4, alpha=1) + 
    annotate("text", x = 10.5, y =100, label = "Scott Jones show\nno admission to Creighton", angle=90, colour="grey70", size=4) + 
  annotate("text", x = 15.5, y = 100, label = "Scott Jones show\nno admission to Creighton", angle=90, colour="grey70", size=4) + 
...
Thanks to R \cite{R}, and in particular the packages knitr \cite{knitr-package, knitr-book1, knitr-chapter}, ggplot2, dplyr, and XXX.
