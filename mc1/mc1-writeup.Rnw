...
\acknowledgments{
The authors wish to thank R Core~\cite{R}, and in particular the authors of the packages knitr~\cite{knitr-package, knitr-book1}, ggplot2~\cite{ggplot2}, and dplyr~\cite{dplyr}.
}
