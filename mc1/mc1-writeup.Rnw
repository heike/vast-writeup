...
For that, we first need to identify when rides at each attraction occur. We are using the time of park-goers' first movement after a check-in event as a potential end time of a ride. For most rides, these end times show huge spikes, indicating the end of a ride. 
...
Using Ward's method for a hierarchical clustering based on these participant-participant distances, we get identify groups of park-goers. 
...
\begin{figure}
<<groups, echo=FALSE, dependson='groupsdata', fig.width=10, fig.height=10, out.width='\\columnwidth', results='asis'>>=
