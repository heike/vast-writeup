...
breaks <- merge(breaks, unique(checkinsgr[, c("id", "group", "day")]), by=c("id", "day"), all.x=TRUE)
...
colour="#FFFFCC", size=I(3)) +
  geom_text(aes(x=dmin+timespent/120, y=id, label=Name), colour="grey30", angle=90, data=subset(breaks, timespent > 10 & id %in% c(1333734, 759761)), size=3.5)
@
