...
breaks$dtime <- cut(breaks$timespent, breaks=c(0,5,15, 60, 120, 1000))
parkX <- read.csv("data/park-xtd.csv")
breaks <- merge(breaks, parkX, by=c("X", "Y"), all=T)
...
\begin{figure}
<<groups, echo=FALSE, dependson='groupsdata', fig.width=10, fig.height=9.5, out.width='\\columnwidth', results='asis', messages=FALSE>>=
...
breaks <- merge(breaks, checkinsgr[, c("id", "group", "day")], by=c("id", "day"), all.x=TRUE)
breaks$id <- factor(breaks$id)

qplot(x=dmin, xend=dmin+timespent/60^2, y=id, yend=id, data=subset(checkinsgr, day=="Sat" & group %in% c(483, 1185, 1323, 1705, 1566, 1819, 1838)), colour=Type,geom="segment", size=I(3)) +  theme_bw() + scale_colour_manual(values=cols[c(3,4,7,8,1,6)]) +
  ...
  scale_x_continuous("Time of the day", breaks=8:23,
                    labels=paste(c(8:12,1:11), c(rep("am", 4), rep("pm", 12))))+
geom_segment(aes(x=dmin, xend=dmin+timespent/60, y=id, yend=id), data=subset(breaks, timespent > 10 & day=="Sat" & group %in% c(483, 1185, 1323, 1705, 1566, 1819, 1838)),
colour="#FFFFCC", size=I(3)) +
  geom_text(aes(x=dmin+timespent/(2*60), y=id, label=Name), colour="grey30", angle=90, data=subset(breaks, timespent > 10 & id %in% c(1333734, 759761)), size=3.5)
@
